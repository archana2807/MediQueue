...
const STATUS_ICONS: Record<string, React.ReactNode> = {
  ...
  CHECKED_IN: <Calendar className="size-3" />,
...
export default function PatientAppointmentsTable() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery<{
    ...
    queryKey: ["patient-appointments", page, pageSize, debouncedSearch],
    queryFn: async () => {
      const response = await fetch(
        `/api/my-appointments/patient?page=${page}&limit=${pageSize}&search=${debouncedSearch}`
      );
      if (!response.ok) {
